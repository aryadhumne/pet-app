...
class Pet(db.Model):
    ...
    gender = db.Column(db.String(10))   # ✅ required
...
def login():
    if request.method == "POST":
        ...
        if user and check_password_hash(user.password, request.form["password"]):
            session["user_id"] = user.id   # 🔥 KEY LINE
            return redirect(url_for("dashboard"))
...
def register():
    if request.method == "POST":
        ...
        print("DEBUG REGISTER DATA:", username, email, phone)
        ...
        # ❌ REMOVE session creation on register
